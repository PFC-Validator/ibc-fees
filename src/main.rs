                ...
                if let Some(fee) = auth.fee {
                    ...
                    if let Some(s) = auth.signer_infos.first() {
                        if let Some(pubkey) = &s.public_key {
                            ...
                            log::debug!("Account:{} {}", account.account()?, s.sequence);
                            let ibc = IBCTx {
                                coins: fee.amount,
...
fn process_block(txs: &Vec<String>) -> anyhow::Result<HashMap<String, HashMap<String, u128>>> {
...
async fn fetch_blocks(terra: &Terra, rpc: &str) -> anyhow::Result<IBCState> {
    ...
    while state.height <= height {
        while state.height <= height {
            ...
            let block_tally = process_block(txs)?;
